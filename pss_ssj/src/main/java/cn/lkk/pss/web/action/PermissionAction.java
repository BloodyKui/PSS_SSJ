...
public class PermissionAction extends CRUDAction<Permission> {
	 ...
	 */
	private PageList<Permission> pageList = new PageList<>();
	...
	public void list() {
		// 将数据通过map栈传到前端页面
		// putContext("permissions", permissionService.getAll());
		// permissions = permissionService.getAll();
		// -1代表选择所有部门，那么就将查询条件对象中的部门id情况，去掉对部门的筛选
		System.out.println(baseQuery);
		pageList = permissionService.findPageByQuery(baseQuery);
	}
	...
	public String delete() throws Exception {
		...
		try {
			permissionService.delete(super.id);
	...
	public PageList<Permission> getPageList() {
