...
public class DepartmentAction extends CRUDAction<Department> {
	 ...
	 */
	private PageList<Department> pageList = new PageList<>();
	...
	public void list() {
		// 将数据通过map栈传到前端页面
		// putContext("departments", departmentService.getAll());
		// departments = departmentService.getAll();
		// -1代表选择所有部门，那么就将查询条件对象中的部门id情况，去掉对部门的筛选
		System.out.println(baseQuery);
		pageList = departmentService.findPageByQuery(baseQuery);
	}
	...
	public String delete() throws Exception {
		...
		try {
			departmentService.delete(super.id);
	...
	public PageList<Department> getPageList() {
