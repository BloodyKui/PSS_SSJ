...
public class SystemDictionaryTypeAction extends CRUDAction<SystemDictionaryType> {
	 ...
	 */
	// 通过get方法传到前端页面
	// private List<SystemDictionaryType> systemDictionaryTypes;

	/*
	 * 创建PageList对象用于将数据返回到页面
	 */
	private PageList<SystemDictionaryType> pageList = new PageList<>();
	...
	public void list() {
		// 将数据通过map栈传到前端页面
		// putContext("systemDictionaryTypes", systemDictionaryTypeService.getAll());
		// systemDictionaryTypes = systemDictionaryTypeService.getAll();
		// -1代表选择所有部门，那么就将查询条件对象中的部门id情况，去掉对部门的筛选
		System.out.println(baseQuery);
		pageList = systemDictionaryTypeService.findPageByQuery(baseQuery);
	}
	...
	public String delete() throws Exception {
		...
		try {
			systemDictionaryTypeService.delete(super.id);
	...
	public void prepareInput() throws Exception {
		if (id != null) {
			...
			// 所以持久层要使用findOne方法
			// systemDictionaryType = systemDictionaryTypeService.get(systemDictionaryType.getId());
	...
	public PageList<SystemDictionaryType> getPageList() {
