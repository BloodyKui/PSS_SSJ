...
public class SystemDictionaryDetailAction extends CRUDAction<SystemDictionaryDetail> {
	 ...
	 */
	// 通过get方法传到前端页面
	// private List<SystemDictionaryDetail> systemDictionaryDetails;

	/*
	 * 创建PageList对象用于将数据返回到页面
	 */
	private PageList<SystemDictionaryDetail> pageList = new PageList<>();
	...
	public void list() {
		// 将数据通过map栈传到前端页面
		// putContext("systemDictionaryDetails", systemDictionaryDetailService.getAll());
		// systemDictionaryDetails = systemDictionaryDetailService.getAll();
		// -1代表选择所有部门，那么就将查询条件对象中的部门id情况，去掉对部门的筛选
		System.out.println(baseQuery);
		pageList = systemDictionaryDetailService.findPageByQuery(baseQuery);
	}
	...
	public String save() throws Exception {
		...
		// 保存后就可以获得这个id
		id = systemDictionaryDetail.getId();
	...
	public String delete() throws Exception {
		...
		try {
			systemDictionaryDetailService.delete(super.id);
	...
	public void prepareInput() throws Exception {
		if (id != null) {
			...
			// 所以持久层要使用findOne方法
			// systemDictionaryDetail = systemDictionaryDetailService.get(systemDictionaryDetail.getId());
	...
	public PageList<SystemDictionaryDetail> getPageList() {
