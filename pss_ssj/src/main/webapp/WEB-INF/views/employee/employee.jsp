		...
		<div class="main-content">
			...
			<div class="page-content">
				<div class="page-header">
					用户名
					<s:textfield name="baseQuery.username" placeholder="Search ..."
						class="nav-search-input" id="nav-search-input" autocomplete="off" />
					邮件
					<s:textfield name="baseQuery.email" placeholder="Search ..."
						class="nav-search-input" id="nav-search-input" autocomplete="off" />
					部门
					<s:select name="baseQuery.deptId" list="#allDept" listKey="id"
						listValue="name" headerKey="-1" headerValue="--所有部门--" />
					...
					<!-- 新增数据按钮 -->
					<a href="/employee_input.action?baseQuery.pageSize=${baseQuery.pageSize}" class="btn btn-grey"> <i
						class="icon-pencil align-top bigger-125"></i> 添加用户
					</a>
					<!-- 下载文件按钮 -->
					<button type="button" onclick="downloadFile('employee')" class="btn btn-pink"> <i
						class="icon-download-alt align-top bigger-125"></i> 导出文件
					</button>
				</div>
				<!-- /.page-header -->
										...
										<tbody id="eachPage">
											<s:iterator value="pageList.results">
												...
												<tr
													...
													<td>${id}</td>
													<td><img alt="头像" src="${headImage}"></td>
													<td>${username}</td>
													<td>${password}</td>
													<td>${email}</td>
													<td>${age}</td>
													...
													<td>
														<div
															...
															<!-- 编辑时为了保证编辑过后留在当前页，需要传递的参数比较多
																 ...
																 的参数
															 -->
															<%-- <a href="/employee_input.action?id=${id}&baseQuery.currentPage=${baseQuery.currentPage}" --%>
															<!-- class="btn btn-xs btn-info"> <i -->
															<!-- class="icon-edit bigger-120"></i> -->
															<!-- </a>  -->
															<button type="button"
																onclick="transferParams('/employee_input.action', '${id}')"
																class="btn btn-xs btn-info">
																<i class="icon-edit bigger-120"></i>
															</button>
															<button type="button"
																onclick="deleteOne('/employee_delete.action', ${id},this)"
