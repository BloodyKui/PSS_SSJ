...
import cn.lkk.pss.service.I${entity}Service;

@Controller
@Scope("prototype")
public class ${entity}Action extends CRUDAction<${entity}> {
	@Autowired
	 ...
	 */
	// 通过get方法传到前端页面
	// private List<${entity}> ${lowerEntity}s;

	/*
	 * 创建PageList对象用于将数据返回到页面
	 */
	private PageList<${entity}> pageList = new PageList<>();

	/*
	 * 查询条件对象，用于封装从前台传过来的查询条件
	 */
	private ${entity}Query baseQuery = new ${entity}Query();
	// 实体类 因为CRUDAction父类实现了ModelDriven方法，所以不用提供get方法，通过getModel拿值
	// 同时在拦截器ModelDriven中如果对象不为空，则将值压入值栈的栈顶。此时在页面上就可以直接使用对象中的字段进行取值了
	private ${entity} ${lowerEntity};

	@Override
	public void list() {
		// 将数据通过map栈传到前端页面
		// putContext("${lowerEntity}s", ${lowerEntity}Service.getAll());
		// ${lowerEntity}s = ${lowerEntity}Service.getAll();
		// -1代表选择所有部门，那么就将查询条件对象中的部门id情况，去掉对部门的筛选
		System.out.println(baseQuery);
		pageList = ${lowerEntity}Service.findPageByQuery(baseQuery);
	}
	...
	public String delete() throws Exception {
		...
		try {
			${lowerEntity}Service.delete(super.id);
	...
	public void prepareInput() throws Exception {
		if (id != null) {
			...
			// 所以持久层要使用findOne方法
			// ${lowerEntity} = ${lowerEntity}Service.get(${lowerEntity}.getId());
			${lowerEntity} = ${lowerEntity}Service.get(id);
		}
	}
	...
	public PageList<${entity}> getPageList() {

		return pageList;
	}

	public ${entity}Query getBaseQuery() {
